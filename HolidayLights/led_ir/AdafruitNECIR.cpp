...
Adafruit_NECremote::Adafruit_NECremote(uint8_t pin) {
  _irpin = pin;
...
int16_t Adafruit_NECremote::listen(int16_t maxwaitmilliseconds) {
  uint16_t p1, p2, timer;
  
  p1 = p2 = 0;
  timer = 0; 
  maxwaiting = maxwaitmilliseconds;
  maxwaiting *= 1000;  // to microseconds
  
  if (maxwaitmilliseconds) timing = true;
  timedout = false;
...
uint16_t Adafruit_NECremote::measurePulse(boolean state) {
  uint16_t pulse = 0;
  uint8_t statemask;
...
uint8_t Adafruit_NECremote::readNECbit(void) {
  int16_t p1, p2;
